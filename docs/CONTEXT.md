...
**Past-day intake + burn backfill (option A):** tap any row in the **Day-by-day log** → inline editor (`hubEditBurn`/`hubSaveDay`/`hubCancelBurn`, state in `hubEditKey`) with THREE fields: **Intake** (`cal_intake_manual`), **Burn** (`cal_exp_manual`), and **Weight** (`daily_logs.weight`, per-day, backfillable here). `dbSaveDay` upsert, **never touches the `oura` jsonb blob**. Intake manual entry now OVERRIDES logged food too (logs are often incomplete — e.g. a 170-kcal day); priority: manual intake > logged food > none. Blank intake reverts to the logged-food total. Editor shows the true logged total (`x.foodCal`) as placeholder/hint. Burn manual overrides Oura. Recomputes everything live. **Conversational backfill ("set May 14 burn to 2900") is option C — PARKED, next pass, same storage.**
